...
export function SpreadsheetImport({ isOpen, onClose, units, onImportComplete }: SpreadsheetImportProps) {
  ...
  const processData = async (data: any[][]) => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const description = formatTitleCase(row[0]?.toString().trim() || "");
      const unitUseName = row[1]?.toString().trim() || "un";
      const rawFactor = parseFloat(row[2]?.toString()) || 1;
      const unitPurchName = row[3]?.toString().trim() || "un";
      ...
      const existingItem = existingItems?.find(
        (existing) => existing.description.toUpperCase() === item.description.toUpperCase(),
      );
      ...
      if (!item.unit_purch_name) {
        ...
        const purchUnit = units.find((u) => u.description.toLowerCase() === item.unit_purch_name.toLowerCase());
        if (purchUnit) {
          item.unit_purch = purchUnit.id;
        } else {
          item.errors.push(
            `Unidade de compra "${item.unit_purch_name}" não encontrada. Certifique-se de que a unidade está cadastrada.`,
          );
        }
      }
      ...
      if (!item.unit_use_name) {
        ...
        const useUnit = units.find((u) => u.description.toLowerCase() === item.unit_use_name.toLowerCase());
        if (useUnit) {
          item.unit_use = useUnit.id;
        } else {
          item.errors.push(
            `Unidade de uso "${item.unit_use_name}" não encontrada. Certifique-se de que a unidade está cadastrada.`,
          );
        }
      }
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>Faça upload de um arquivo CSV ou Excel com os dados dos insumos</DialogDescription>
        </DialogHeader>
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {item.errors.length === 0 ? (
                            <Badge
                              variant={item.isUpdate ? "secondary" : "default"}
                              className={item.isUpdate ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"}
                            >
                      ...
                      {invalidItems.map((item, index) => (
                        <div key={index} className="border-l-4 border-red-500 pl-3">
                          <p className="font-medium">
                            Linha {item.rowIndex}: {item.description}
                          </p>
