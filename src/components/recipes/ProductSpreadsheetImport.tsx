...
export function ProductSpreadsheetImport({ isOpen, onClose, onImportComplete }: ProductSpreadsheetImportProps) {
  ...
  const processData = async (data: any[][]) => {
    ...
    const { data: existingProducts } = await supabase.from("recipe").select("id, description");

    const { data: existingItems } = await supabase.from("item").select("id, description");

    const { data: existingProductItems } = await supabase.from("recipe_item").select("recipe, item");
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const existingProduct = existingProducts?.find(
        (product) => product.description.toUpperCase() === item.productDescription.toUpperCase(),
      );
      ...
      const existingItem = existingItems?.find(
        (itemRecord) => itemRecord.description.toUpperCase() === item.itemDescription.toUpperCase(),
      );
      ...
      if (item.productId && item.itemId) {
        const existingProductItem = existingProductItems?.find(
          (ri) => ri.recipe === item.productId && ri.item === item.itemId,
        );
      ...
      for (const [productKey, productItems] of productGroups) {
        ...
        for (const item of productItems) {
          if (item.isProductItemUpdate) {
            ...
            await supabase.from("recipe_item").insert([
              {
                recipe: productId,
                item: item.itemId!,
                qty: item.qty,
  ...
  const newProducts = [
    ...new Set(validItems.filter((item) => !item.isProductUpdate).map((item) => item.productDescription)),
  ];
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>Faça upload de um arquivo CSV ou Excel com produtos e seus insumos</DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <br />
                  <Button variant="link" className="p-0 h-auto font-normal" onClick={downloadTemplate}>
                  ...
                  {isProcessing ? (
                    ...
                    <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">Formatos suportados: CSV, Excel (.xlsx)</p>
                        <input
                          type="file"
                          accept=".csv,.xlsx,.xls"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {newProducts.length} produtos novos, {updatedProducts.length} produtos atualizados,{" "}
                    {newProductItems.length} itens novos, {updatedProductItems.length} itens atualizados,{" "}
                    {invalidItems.length} com erro
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button onClick={handleImport} disabled={validItems.length === 0 || isProcessing}>
                    {isProcessing ? (
                      <>
                        ...
                        <TableCell>
                          {item.errors.length === 0 ? (
                            <div className="space-y-1">
                              <Badge
                                variant={item.isProductUpdate ? "secondary" : "default"}
                                className={
                                  item.isProductUpdate ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"
                                }
                              >
                                {item.isProductUpdate ? "Produto: Atualizar" : "Produto: Criar"}
                              </Badge>
                              <Badge
                                variant={item.isProductItemUpdate ? "secondary" : "default"}
                                className={
                                  item.isProductItemUpdate ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"
                                }
                              >
                      ...
                      {invalidItems.map((item, index) => (
                        <div key={index} className="border-l-4 border-red-500 pl-3">
                          <p className="font-medium">
                            Linha {item.rowIndex}: {item.productDescription} - {item.itemDescription}
                          </p>
